...
def setAtributesMeanTest():
    try:
        ...
        return render_template('mean_test.html', randomNumbers=meanTestClass.randomNumbers, acept=meanTestClass.acept, error=meanTestClass.error,   n=meanTestClass.numberN, r=constants.FORMAT_NUMBER.format(meanTestClass.calculateMean()), alpha=meanTestClass.alpha, z=constants.FORMAT_NUMBER.format(meanTestClass.z), li=constants.FORMAT_NUMBER.format(meanTestClass.calculateLI()), ls=constants.FORMAT_NUMBER.format(meanTestClass.calculateLS()), verify=meanTestClass.verifyTest(), grafic=encoded_img)
    except IOError:
        return meanTest()
